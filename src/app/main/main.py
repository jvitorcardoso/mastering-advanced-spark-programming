...
def main():
    try:
        with open("/opt/bitnami/spark/jobs/app/main/config/config.json", "r") as config_file:
            config = json.load(config_file)

        spark = create_spark_session(app_name="yelp-etl-job")
        sc = spark.sparkContext
        sc.setJobGroup("yelp-etl-job", "ETL Job for Yelp Dataset")
        ...
        # TODO Data Output
        write_data(df_join, config["output_path"] + "/yelp-business", "parquet")
...
if __name__ == "__main__":
    logger.info("ETL Job for Yelp Dataset Started")
    main()
    logger.info("ETL Job for Yelp Dataset Completed")
